...
A `working` report is retired to `unknown` if it ages out while the pane stays silent, because a run in flight keeps repainting its pane. This bounds a lost report: one skipped or dropped transition no longer leaves a finished pane reading as working. `agent list` shows that pane as `unknown`, and `agent explain` shows `stale_report` with the `reported_state` and `reported_age_ms` behind the retirement. `idle` and `blocked` reports are never retired; only an accurate new report replaces them.
